...
class SistemaPedidos {  //controller
  ...
  atualizarProdutos(){
    ...
    criarPedido() {
      ...
      const cliente = this.clientes.find(c => c.id === clienteId);//percorrer o cliente na lista
    ...
    this.atualizarPedidos(); //atualiza a lista de pedidos no html
    ...
    atualizarPedidos(){
      ...
      this.pedidos.forEach(pedido => {
        const li = document.createElement("li");
        li.innerText = `Pedido ${pedido.id} - Cliente: ${pedido.cliente.getNome()} - Total: R$ ${pedido.total.toFixed(2)}`;
        lista.appendChild(li);
      }); 
    }
}
